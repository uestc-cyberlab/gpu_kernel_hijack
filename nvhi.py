...
def nv_generate(arch):
    if arch == 'Pascal':
        instruction = [-0x01,-0x01,0x00,0x00,0x00,0x00,0x00,0x00]
        for i in range(256):
        ...
        instruction=[0x00,0x00,0x00,0x00,0x00,0x00,-0x01,-0x01,  -0x01,0x00,0x00,0x00,0x00,0x00,0x00,0x00]
        for i in range(256):
            instruction[6] = instruction[6] + 0x01
            for j in range(256):
                instruction[7] = instruction[7] + 0x01
                yield instruction
            instruction[7]=-0x01
            
    elif arch == 'Ampere':
        instruction=[0x00,0x00,0x00,0x00,0x00,0x00,-0x01,-0x01,  0x00,0x4f,0xca,0x00,0x00,0x00,0x00,0x00]
        for i in range(16):
            instruction[6] = instruction[6] + 0x01
            for j in range(256):
                instruction[7] = instruction[7] + 0x01
...
'''
def nv_save(save_path, instruction, dump_en, assemble):
    ...
    if dump_en == 1: #可反汇编的隐藏指令
        write_context = [assemble, ins_string]#
    else: #不可反汇编的隐藏指令
        write_context = ['non-disassemble', ins_string]#
...
#暗指令挖掘主要流程
    if 1:
        file_path = r'.\cuda_project\encoding\kernel.cubin'
        save_path = r'search.csv'
        base = 0x497A0 #待测指令插入在第一行
        it = nv_generate()
        while True: 
            try:
                #1.生成待测指令
                ...
                #2.写入待测指令到可执行文件
                res = nv_write(instruction, file_path, offset=0x0) 
                #print(res)
                ...
                #4.反汇编可执行文件
                if res == 0: #可反汇编
                    res,assemble = nv_extract(stdout, line='0x00') #测试指令插在指令流的第一行
                else:#不可反汇编
